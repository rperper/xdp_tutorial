			...
			struct stats_record *stats_prev)
{
    ...
    for (enum xdp_action key = XDP_ABORTED; key <= XDP_REDIRECT; ++key)
	{
		char *fmt = "%-12s %'11lld pkts (%'10.0f pps)"
			" %'11lld bytes (%'10.0f bytes/s)"
			" period:%f\n";
		...
		printf(fmt, action, rec->total.rx_packets, pps, bytes, (double)(bytes / period), period);
	}
}
